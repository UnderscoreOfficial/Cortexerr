...
public sealed class Sabnzbd(string nzb_link)
{
    ...
    private DateTime _added_timestamp { get; set; }
    private int _retry_instance { get; set; } = 0;
    private bool _history { get; set; } = false;
    public string nzb_link { get; } = nzb_link;
    public string? nzo_id { get; private set; }

    async private Task<HandleResponse<T>> Request<T>(string endpoint, bool log = true)
    {
    ...
    async private Task<HandleResponse<SabnzbdHistoryResponseSlot>> History(bool log = true)
    {
        if (nzo_id == null)
        ...
        var request = await Request<SabnzbdHistoryResponse>(endpoint, log);
        if (request.error != null) return Response.Error<SabnzbdHistoryResponseSlot>(request.error.code, request.error.message, log);

        if (request.data?.history?.slots?.Length == 0)
            return Response.Error<SabnzbdHistoryResponseSlot>(ErrorCode.NOT_FOUND, "(Sabnzbd|History) No history items found", log);
    ...
    async private Task<HandleResponse<SabnzbdQueueResponseSlot>> Queue(bool log = true)
    {
        if (nzo_id == null)
        ...
        var request = await Request<SabnzbdQueueResponse>(endpoint, log);
        if (request.error != null) return Response.Error<SabnzbdQueueResponseSlot>(request.error.code, request.error.message, log);

        if (request.data?.queue?.slots?.Length == 0)
            return Response.Error<SabnzbdQueueResponseSlot>(ErrorCode.NOT_FOUND, "(Sabnzbd|Queue) No queue items found", log);
        var nzb = Error.Handle<SabnzbdQueueResponseSlot?>(() => request.data?.queue?.slots?[0]);
        if (nzb.error != null) return Response.Error<SabnzbdQueueResponseSlot>(ErrorCode.UNEXPECTED_ERROR, nzb.error.message);
        if (nzb.data != null)
        {
            return Response.Success(nzb.data);
        }
    ...
    async public Task<HandleResponse<SabnzbdSlotResponse>> Nzb(bool polling = true)
    {
        ...
        if ((DateTime.UtcNow - _added_timestamp).TotalMilliseconds < 200)
            await Task.Delay(200);
        HandleResponse<SabnzbdQueueResponseSlot>? queue = null;
        HandleResponse<SabnzbdHistoryResponseSlot>? history = null;


        if (!_history)
        {
            queue = await Queue(false);
        }

        if (_history || queue?.error?.code == ErrorCode.NOT_FOUND)
        {
    ...
    async public Task<HandleResponse<SabnzbdStatusResponse>> Add()
    {
        ...
        {
            nzo_id = _nzo_id;
            _added_timestamp = DateTime.UtcNow;
            return request;
        }
